...
var colorList = Vue.component('color-list', {
  template: '<div class="color-list">' +
    '<div class="list">' +
      '<div class="color" v-for="rgb in rgbList" v-bind:style="{ backgroundColor: rgb }" v-on:click="onClickColor(rgb)">' +
      '</div>' +
    '</div>' +
    '<selected-color :selectedColorHex="colorHex"></selected-color>' +
  '</div>',
  data : function() {
    return {
      rgbList: [
        ...
        "#000000", "#282828", "#323232", "#3c3c3c"
      ],
      colorHex: ""
    };
  },
  methods: {
    onClickColor: function (rgbHex) {
      console.log("click color", rgbHex);
      this.colorHex = rgbHex;
    }
  }
});

var selectedColor = Vue.component('selected-color', {
  template: '<div class="selected-color">' +
    '<div class="selected">' +
      '<input type="text" class="hex" v-model="selectedColorHex"/>' +
      '<div class="color" v-bind:style="{ backgroundColor: selectedColorHex }"></div>' +
    '</div>' +
  '</div>',
  props: ['selectedColorHex']
});
